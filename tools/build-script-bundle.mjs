...
function generatedEntry() {
  const definitions = allModules.map(module => {
...
async function validateManifest() {
  assert.ok(allModules.length > 0, '模块清单不能为空');
  assert.equal(new Set(allModules.map(module => module.id)).size, allModules.length, '模块 ID 存在重复');
  assert.ok(!allModules.some(module => module.id.includes('monopoly')), '大富翁不应进入本阶段构建');
  ...
  for (const module of allModules) {
    if (!module.source) {
      assert.ok(module.entry, `产品模块必须提供入口：${module.name}`);
      const entryStat = await fs.stat(path.join(projectRoot, module.entry));
      assert.ok(entryStat.isFile(), `产品模块入口不存在：${module.entry}`);
...
const buildManifest = {
  ...
  modules: allModules.map(({ id, name, source, entry, requires = [], provides = [], legacyRequires = [], features }) => ({
    id,
    name,
    source: source ?? null,
    entry: entry ?? source,
    implementation: source ? (entry ? 'infrastructure-adapter' : 'faithful-source') : 'product-module',
    requires,
    provides,
    legacyRequires,
    features: migrationFeatureContract[id] ?? features ?? [],
  })),
...
console.log(
  `${checkOnly ? '多合一检查通过' : '多合一构建完成'}：${moduleManifest.length} 个原貌模块 + ${productModuleManifest.length} 个产品模块，${bundle.byteLength} 字节`,
);
