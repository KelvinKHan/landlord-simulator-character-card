import { jsDelivrSources } from '../release-config.mjs';

export function createJsDelivrLoader(config) {
  const sources = jsDelivrSources({
    repository: config.repository,
    ref: config.gitTag,
    path: config.bundlePath,
  });

  return `// 房东模拟器多合一在线加载器 ${config.gitTag}
...
const landlordSources = ${JSON.stringify(sources, null, 2)};
const landlordLoaderState = {
  version: ${JSON.stringify(config.version)},
  release: ${JSON.stringify(config.gitTag)},
  status: 'loading',
  loadedFrom: null,
  attempts: [],
  error: null,
