...
- `chat.database`：聊天隔离、会话、消息、撤回和导入导出；
- `chat.core`：组织上下文、调用 AI、解析群聊与私聊回复；
- `chat.sync`：把聊天摘要同步到当前 SillyTavern 聊天的世界书；
- `chat.app`：微信界面和用户操作，只调用前三项服务，不直接碰数据库或世界书实现。

界面又拆为控制器、纯 HTML 渲染函数、独立 CSS 和表情包仓库。以后可以换 UI、换 AI 生成方式或换存储，而不必重写整套聊天。
